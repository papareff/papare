"""反讽任务辩论消融实验——立场中立化版本（独立运行）。

与 run_irony_debate.py 使用完全相同的 150 条分层抽样样本（SEED=42），
便于与预设立场版结果直接配对对比。

辩论模式：neutral=True
  - 双方先各自独立判断（ICL 分类，不预设立场）
  - 意见一致直接采纳；分歧时各自为自己的判断辩护，可被说服翻转
对比基线：
  - no_debate：仅微调裁决者独立判决（与预设立场实验相同基线）
...
config['debate']['neutral'] = True   # 立场中立化
...
print(f"辩论阶段样本: {len(deb_samples)}（与预设立场实验相同批次）")
...
print(f"\n===== with_debate_neutral（{len(deb_samples)} 条）=====")
rows, correct = [], 0
...
pd.DataFrame(rows).to_csv("experiments/results/irony_debate_neutral_with.csv", index=False)
print(f"  → with_debate_neutral: acc={acc_d:.2%}")
summary.append({"stage": "debate_neutral", "condition": "with_debate", "accuracy": acc_d,
...
pd.DataFrame(rows).to_csv("experiments/results/irony_debate_neutral_without.csv", index=False)
print(f"  → no_debate: acc={acc_nd:.2%}")
summary.append({"stage": "debate_neutral", "condition": "no_debate", "accuracy": acc_nd,
                "n": len(deb_samples), "avg_latency": np.mean([r["latency_sec"] for r in rows])})
...
pd.DataFrame(summary).to_csv("experiments/results/irony_debate_neutral_summary.csv", index=False)
print("结果已保存到 experiments/results/irony_debate_neutral_*.csv")
